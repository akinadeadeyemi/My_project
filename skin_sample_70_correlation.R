...
# final_skin_samples_70 <- final_skin_metadata %>%
#  filter(species %in% filtered_species)

# write out the final blood samples
# write.csv(final_skin_samples_70, "final_skin_samples_70.csv", row.names = FALSE)


####### merging sample IDs in both beta and blood_skin_samples to generate  ########
# skin_metadata_betas70 <- skin_metadata_betas1[, colnames(skin_metadata_betas1) %in% final_skin_samples_70$geo_accession]
# View(skin_metadata_betas70)

# write.csv(skin_metadata_betas70, "skin_metadata_betas70.csv", row.names = FALSE)

#### Ensure metadata is filtered for matching IDs
# final_skin_samples_70$age_years <- as.numeric(as.character(final_skin_samples_70$age_years))
...
cat(" file loading ...")
final_skin_samples_70 <- read_csv("~/Phd_data/final_skin_samples_70.csv")
skin_metadata_betas70 <- read_csv("~/Phd_data/skin_metadata_betas70.csv")
...
skin_sample_70_corr_test <- correlation_test(
  data = skin_metadata_betas70,  # The beta values of the CpG sites
  metadata = final_skin_samples_70,  # The metadata data frame
  n_samples = 70,  # Number of individuals to sample
  reps = 100  # Number of bootstrap repetitions
)

cat("Saving the result to the folder")

saveRDS(skin_sample_70_corr_test, file = "~/Phd_data/outputs/skin_sample_70_correlation.rds")

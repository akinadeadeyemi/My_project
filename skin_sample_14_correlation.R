...
# final_skin_samples_14 <- final_skin_metadata %>%
#  filter(species %in% filtered_species)

# write out the final blood samples
# write.csv(final_skin_samples_14, "final_skin_samples_14.csv", row.names = FALSE)


####### merging sample IDs in both beta and blood_skin_samples to generate  ########
# skin_metadata_betas14 <- skin_metadata_betas1[, colnames(skin_metadata_betas1) %in% final_skin_samples_14$geo_accession]
# View(skin_metadata_betas14)

# write.csv(skin_metadata_betas14, "skin_metadata_betas14.csv", row.names = FALSE)

#### Ensure metadata is filtered for matching IDs
# final_skin_samples_14$age_years <- as.numeric(as.character(final_skin_samples_14$age_years))
...
final_skin_samples_14 <- read_csv("~/Phd_data/final_skin_samples_14.csv")
skin_metadata_betas14 <- read_csv("~/Phd_data/skin_metadata_betas14.csv")
...
skin_sample_14_corr_test <- correlation_test(
  data = skin_metadata_betas14,  # The beta values of the CpG sites
  metadata = final_skin_samples_14,  # The metadata data frame
  n_samples = 14,  # Number of individuals to sample
  reps = 100  # Number of bootstrap repetitions
)

cat("Saving the result to the folder")

saveRDS(skin_sample_14_corr_test, file = "~/Phd_data/outputs/skin_sample_14_corr_test.rds")
